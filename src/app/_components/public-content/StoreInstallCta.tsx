...
type Props = {
  ...
  description?: string;
  compact?: boolean;
  /** Legacy option: the mobile download shortcut now stays in document flow. */
...
export function StoreBadge({ store, surface }: { store: Store; surface: StoreCtaSurface }) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mt-6 flex flex-col flex-wrap items-center justify-center gap-3 min-[390px]:flex-row">
...
function MobileStoreInstallBar() {
  return (
    <aside className="mt-4 flex items-center gap-3 rounded-[18px] border border-[#E7DDF7] bg-white p-3 sm:hidden" aria-label="썸타임 앱 설치">
      <Image src="/images/logo.png" width={42} height={42} alt="" className="h-10 w-10 shrink-0 rounded-[11px]" />
      ...
      <Link href="/download" className="inline-flex min-h-11 shrink-0 items-center rounded-[12px] bg-[#7A4AE2] px-4 text-[13px] font-bold text-white">앱 다운로드</Link>
    </aside>
  );
}
